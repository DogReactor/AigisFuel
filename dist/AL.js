...
const { decompress } = require('mini-lz4');
    ...
    Package(path) {
        ...
        for (let i = 0; i < this.Count; i++) {
            ...
            if (this.Vers === 3) {
                ...
                entry.Unknown2.copy(newBuffer, offset, 0, entry.Unknown2.byteLength);
                offset += entry.Unknown2.byteLength;
                const stringBuffer = Buffer.from(entry.Name + '\0');
...
(function (ALAR) {
    class Entry {
        constructor() {
            ...
            this.Name = '';
            this.Unknown3 = 0;
            this.Content = new DefaultAL(Buffer.alloc(0));
...
class ALTX extends AL {
    constructor(buffer) {
        ...
        if (this.Form === 0) {
            ...
            for (let i = 0; i < this.Count; ++i) {
                let frameName = '';
                if (br.Position === blockStart[i] - 0x20 || (i > 0 &&
